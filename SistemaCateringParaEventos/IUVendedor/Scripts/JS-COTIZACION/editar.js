...
$('#tablaCotizacion tbody').on('click', '.btn-editar', function () {
    ...
    console.log("Fila seleccionada para editar:", data);
    abrirModal(data)
...
// Cuando se hace click en "crearNuevo"
...
document.getElementById("btnEditarSeleccion").addEventListener("click", async () => {
    ...
    try {
        ...
        console.log("Respuesta del controller:", result);

        if (result.resultado) {
            swal("¡Editado!", result.mensaje, "success");
            tabledata.ajax.reload(null, false); // false evita que vuelva a la primera página
            // Buscamos y actualizamos la fila del DataTable
            const fila = tabledata.rows().data().toArray().find(f => f.IdCotizacion === idCotizacion);
            const rowIndex = tabledata.rows().indexes().toArray().find(i => tabledata.row(i).data().IdCotizacion === idCotizacion);

            if (rowIndex !== undefined) {
                // actualizás los campos modificados
...
$('#tablaCotizacion tbody').on('click', '.btn-eliminar', function () {
    ...
    console.log("Fila seleccionada para eliminar:", data);
    ...
    swal({
        ...
        try {
            ...
            swal("Error", "Ocurrió un error al intentar eliminar la cotización.", "error");
        }
    });
});
